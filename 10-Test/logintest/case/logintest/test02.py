...
class Test(unittest.TestCase):
    ...
    def test_login_success(self):
        '''用户名、密码正确'''
        self.login('Healer-', 'QIANzhen1969728*')  # 正确用户名和密码
        time.sleep(10)  # 这里等待10秒的原因是由于cnblog存在验证机制，不想在验证码上花过多时间写代码，博主在等待时间手动处理。
        link = self.dr.find_element_by_id('lnk_current_user')
        self.assertTrue('Healer-' in link.text)  # 用assertTrue(x)方法来断言  bool(x) is True 登录成功后用户昵称在lnk_current_user里

    def test_login_pwd_error(self):
        '''用户名正确、密码不正确'''
        self.login('Healer-', 'pwerror')  # 正确用户名，错误密码
        time.sleep(10)  # 这里等待10秒的原因是由于cnblog存在验证机制，不想在验证码上花过多时间写代码，博主在等待时间手动处理。
        ...
        self.dr.get_screenshot_as_file("/Users/healer/Desktop/tt/login_pwd_error.jpg")

    def test_login_pwd_null(self):
        '''用户名正确、密码为空'''
        self.login('Healer-', '')  # 密码为空
        ...
        self.dr.get_screenshot_as_file("/Users/healer/Desktop/tt/login_pwd_null.jpg")

    def test_login_user_error(self):
        '''用户名错误、密码正确'''
        self.login('usernameerror', 'QIANzhen1969728*')  # 密码正确，用户名错误
        time.sleep(10)  # 这里等待10秒的原因是由于cnblog存在验证机制，不想在验证码上花过多时间写代码，博主在等待时间手动处理。
        error_message = self.dr.find_element_by_id('tip_btn').text
        self.assertIn('*该用户不存在', error_message)  # 用assertIn(a,b)方法来断言 a in b
        self.dr.get_screenshot_as_file("/Users/healer/Desktop/tt/login_user_error.jpg")

    def test_login_user_null(self):
        '''用户名为空、密码正确'''
        self.login('', 'QIANzhen1969728*')  # 用户名为空，密码正确
        ...
        self.dr.get_screenshot_as_file("/Users/healer/Desktop/tt/login_user_null.jpg")
